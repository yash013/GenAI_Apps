...
from gen_newzletter.tools.research import SearchAndContents, FindSimilar, GetContents, GetImages
...
# from gen_newzletter.tools.custom_tool import MyCustomTool

# Check our tools documentations for more information on how to use them
# from crewai_tools import SerperDevTool

@CrewBase
class GenNewzletterCrew():
	"""GenNewzletter crew"""
	...
	def editor(self) -> Agent:
		return Agent(
			...
			# llm=self.llm(),
   			step_callback=lambda step: self.step_callback(step, "Cheif Editor"),
		)
	...
	def graphic_designer(self) -> Agent:
		return Agent(
			...
			verbose=True,
			tools=[GetImages()],
			step_callback=lambda step: self.step_callback(step, "Graphic Designer")
		)
	...
	def designer(self) -> Agent:
		return Agent(
			...
			# llm=self.llm(),
   			step_callback=lambda step: self.step_callback(step, "Newsletter Designer"),
		)
	...
	@crew
	def crew(self) -> Crew:
		"""Creates the GenNewzletter crew"""
		return Crew(
